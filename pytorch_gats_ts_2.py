...
class GATs(Model):
    ...
    def fit(
        ...
        torch.save(self.GAT_model, save_path)
...
class GATModel(nn.Module):
    def __init__(self, d_feat=6, hidden_size=64, num_layers=2, dropout=0.0, base_model="GRU"):
        ...
        self.transformation2 = nn.Linear(self.hidden_size, self.hidden_size)
        self.a2 = nn.Parameter(torch.randn(self.hidden_size * 2, 1))
    ...
    def cal_attention2(self, x, y):
        x = self.transformation2(x)
        y = self.transformation2(y)
        ...
        e_y = torch.transpose(e_x, 0, 1)
        attention_in = torch.cat((e_x, e_y), 2).view(-1, dim * 2)
        self.a_t2 = torch.t(self.a2)
        attention_out = self.a_t2.mm(torch.t(attention_in)).view(sample_num, sample_num)
        attention_out = self.leaky_relu(attention_out)
        att_weight = self.softmax(attention_out)
        return att_weight
    ...
    def forward(self, x):
        ...
        # k=2
        att_weight2 = self.cal_attention2(hidden, hidden)
        hidden = att_weight2.mm(hidden) + out[:, -1, :] # 加入k=0最原始的自环out[:, -1, :]
        # fc
